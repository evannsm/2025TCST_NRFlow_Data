...
function plot_together(names, NR_df, MPC_df, plane, col, axs, nrcutoff, mpccutoff)
...
%     mpccutoff = 7;
    ...
    if strcmp(plane, 'xy')
        ...
        subplot(2, 7, col);  % First row in the column (NR)
        ...
        subplot(2, 7, 7 + col);  % Second row in the column (MPC)
        plot(MPC_df_filtered.x, MPC_df_filtered.y, 'red'); hold on;
        plot(MPC_df_filtered.x_ref, MPC_df_filtered.y_ref, '--b');
        xlabel('x (m)'); ylabel('y (m)');
        title([labels{7 + col}, names{2}]);  % Add label before title for MPC
